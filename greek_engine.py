...
trade_type_map = {
    "IRS": "rate_swap.VSwapPricer",
    "Bermudan": "rate_swap.BermudanPricer",
    "FXOption": "fx_option.FXOptionPricer",
}

class GreekEngine(object):
    ...
    def create_instrument(self, trade_data, market_data):
        ...
        pricer_cls = getattr(__import__(str(cls_str[0])), str(cls_str[1]))
        self.instrument = pricer_cls(trade_data, market_data)
    ...
    def get_all_scens(self):
        ...
        for greek in self.req_greeks:
            ...
            if item[0] == 'pv':
                ...
                scens = [('IRYCurve_' + item[1], self.ycdelta_shift), ("MarketDate", 0), ('IRYCurve_' + item[1], -self.ycdelta_shift) ]
            elif item[0] == 'swnvega':
                scens = [('SWNVOL_' + item[1], self.swnvega_shift), ('SWNVOL_' + item[1], -self.swnvega_shift) ]
            elif item[0] in ['fxdelta', 'fxgamma']:
                scens = [('FXFwd_' + item[1], self.fxdelta_shift), ("MarketDate", 0), ('FXFwd_' + item[1], -self.fxdelta_shift) ]
            elif item[0] == 'fxvega':
        ...
        for greek in self.req_greeks:
            ...
            if item[0] == 'pv':               
                ...
                self.calc_risks[greek] = (self.results[('IRYCurve_' + item[1], self.ycdelta_shift)] \
                                        - self.results[('IRYCurve_' + item[1], -self.ycdelta_shift)])/2.0/(self.ycdelta_shift*10000)
            elif item[0] == 'ycgamma':
                self.calc_risks[greek] = (self.results[('IRYCurve_' + item[1], self.ycdelta_shift)] - 2 * self.results[("MarketDate", 0)]\
                                        + self.results[('IRYCurve_' + item[1], - self.ycdelta_shift)])/((self.ycdelta_shift*10000)**2)
                ...
                self.calc_risks[greek] = (self.results[('FXFwd_' + item[1], self.fxdelta_shift)] \
                                        - self.results[('FXFwd_' + item[1], -self.fxdelta_shift)])/(2*self.fxdelta_shift)
            elif item[0] == 'fxgamma':
                self.calc_risks[greek] = (self.results[('FXFwd_' + item[1], self.fxdelta_shift)] - 2 * self.results[("MarketDate", 0)]\
                                        + self.results[('FXFwd_' + item[1], - self.fxdelta_shift)])/(self.fxdelta_shift**2)                                        
            elif item[0] == 'swnvega':
                self.results[greek] = (self.results[('SWNVOL_' + item[1], self.swnvega_shift)] \
                                        - self.results[('SWNVOL_' + item[1], - self.swnvega_shift)])/(2 * self.swnvega_shift*100.0)
            elif item[0] == 'fxvega':
                self.calc_risks[greek] = (self.results[(('FXVOL_' + item[1], 'ATM'), self.fxvega_shift)] \
                                        - self.results[(('FXVOL_' + item[1], 'ATM'), - self.fxvega_shift)])/(2 * self.fxvega_shift*100.0)
